package me.hardicine.task.entity;
...
public class CustomEntityIntArray {
  ...
  public CustomEntityIntArray(int... incomingData) throws CustomArrayException {
    if (incomingData == null) {
      throw new CustomArrayException("Array reference is null");
    }
    if (incomingData.length == 0) {
      throw new CustomArrayException("Array is empty (length 0)");
    }
  ...
  public CustomEntityIntArray(CustomEntityIntArray entityArray) throws CustomArrayException {
    if (entityArray == null) {
      throw new CustomArrayException("Entity array reference is null");
    }
  ...
  public int getElement(int index) throws CustomArrayException {
    if (index < 0) {
      throw new CustomArrayException("Index cannot be negative: " + index);
    }
    if (index >= data.length) {
      throw new CustomArrayException("Index " + index + " is out of bounds for length " + data.length);
    }
